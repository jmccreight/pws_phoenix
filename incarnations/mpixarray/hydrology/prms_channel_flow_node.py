...
class PRMSChannelFlowNode:
    ...
    fields = {
        ...
        # restart=True: advance_type reads seg_inflow's finalized
        # daily mean (-> inflow_ts_prev), and the muskingum recursion
        # reads outflow_ts across the day boundary; the other routing
        # fields are zeroed in prepare or regenerated by advance
        # (matches pywatershed's channel restart list)
    ...
    def initialize_type(dataset, n_substeps, io_in_cfs) -> None:
        ...
        # io_in_cfs unused: muskingum is linear in flow (unit-agnostic)
        for name in (
            "seg_inflow",
            "seg_outflow",
            "inflow_ts",
